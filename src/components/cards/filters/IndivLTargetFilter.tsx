        ...
        <PaddedDiv key={item.key}>
          ...
          {isAttributeTarget(item.target) && (
            <RightMarginSelect
              ...
              onChange={(event) => {
                ...
                if (selected === 'all') editIndivLTarget(item.key, { ...item.target, attributeId: null });
                else editIndivLTarget(item.key, { ...item.target, attributeId: Number(selected) });
              }}
            >
          ...
          {isRoleTarget(item.target) && (
            <RightMarginSelect
              ...
              onChange={(event) => {
                const selected = event.target.value;
                if (selected === 'all') editIndivLTarget(item.key, { ...item.target, roleId: null });
                else editIndivLTarget(item.key, { ...item.target, roleId: Number(selected) });
              }}
            >
              <option value="all">모두</option>
          ...
          {isGroupTarget(item.target) && (
            <RightMarginSelect
              ...
              onChange={(event) => {
                const selected = event.target.value;
                if (selected === 'all') editIndivLTarget(item.key, { ...item.target, groupId: null });
                else editIndivLTarget(item.key, { ...item.target, groupId: Number(selected) });
              }}
            >
              <option value="all">모두</option>
          ...
          {isGradeTarget(item.target) && (
            <RightMarginSelect
              ...
              onChange={(event) => {
                const selected = event.target.value;
                if (selected === 'all') editIndivLTarget(item.key, { ...item.target, grade: null });
                else editIndivLTarget(item.key, { ...item.target, grade: Number(selected) });
              }}
            >
              <option value="all">모두</option>
              {[1, 2, 3].map((grade) => (
          ...
          {isUnitTarget(item.target) && (
            <RightMarginSelect
              ...
              onChange={(event) => {
                const selected = event.target.value;
                if (selected === 'all') editIndivLTarget(item.key, { ...item.target, unitId: null });
                else editIndivLTarget(item.key, { ...item.target, unitId: Number(selected) });
              }}
            >
              <option value="all">모두</option>
