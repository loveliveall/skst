          ...
          <PaddedDiv key={item.key}>
            ...
            {isAttributeTarget(target) && (
              <RightMarginSelect
                ...
                onChange={(event) => {
                  ...
                  if (selected === 'all') editIndivPTarget(item.key, { ...target, attributeId: null });
                  else editIndivPTarget(item.key, { ...target, attributeId: Number(selected) });
                }}
              >
            ...
            {isRoleTarget(target) && (
              <RightMarginSelect
                ...
                onChange={(event) => {
                  const selected = event.target.value;
                  if (selected === 'all') editIndivPTarget(item.key, { ...target, roleId: null });
                  else editIndivPTarget(item.key, { ...target, roleId: Number(selected) });
                }}
              >
                <option value="all">모두</option>
            ...
            {isGroupTarget(target) && (
              <RightMarginSelect
                ...
                onChange={(event) => {
                  const selected = event.target.value;
                  if (selected === 'all') editIndivPTarget(item.key, { ...target, groupId: null });
                  else editIndivPTarget(item.key, { ...target, groupId: Number(selected) });
                }}
              >
                <option value="all">모두</option>
            ...
            {isGradeTarget(target) && (
              <RightMarginSelect
                ...
                onChange={(event) => {
                  const selected = event.target.value;
                  if (selected === 'all') editIndivPTarget(item.key, { ...target, grade: null });
                  else editIndivPTarget(item.key, { ...target, grade: Number(selected) });
                }}
              >
                <option value="all">모두</option>
                {[1, 2, 3].map((grade) => (
            ...
            {isUnitTarget(target) && (
              <RightMarginSelect
                ...
                onChange={(event) => {
                  ...
                  else editIndivPTarget(item.key, { ...target, unitId: Number(selected) });
                }}
              >
                <option value="all">모두</option>
